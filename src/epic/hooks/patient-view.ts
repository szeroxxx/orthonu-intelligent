...
const SERVICE_ID = 'orthonu-oral-intelligence';
const SLOW_THRESHOLD_MS = 400;

export async function patientViewHandler(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const result = await evaluate(request, 'patient-view');
    ...
    if (suppression.suppressed) {
      await insertInvocation({
        ...
        icd10Codes: result.type !== 'no-match' ? (result as { icd10Codes?: string[] }).icd10Codes : undefined,
        cardCount: 0,
        suppressed: true,
        suppressionReason: suppression.reason,
        responseTimeMs: durationMs,
      });
      logInvocation(hookInstance, SERVICE_ID, 'patient-view', context.patientId, 0, true, durationMs);
    ...
    if (result.type === 'no-match') {
      ...
      logInvocation(hookInstance, SERVICE_ID, 'patient-view', context.patientId, 0, false, durationMs);
      res.json({ cards: [] } satisfies CdsServiceResponse);
      return;
    }

    if (result.type === 'passive') {
      const passiveCard = buildPassiveCard(hookInstance);
      await insertInvocation({
        hookInstance,
        hookType: 'patient-view',
        serviceId: SERVICE_ID,
      ...
      logInvocation(hookInstance, SERVICE_ID, 'patient-view', context.patientId, 1, false, durationMs);
    ...
    const ctx: PlaceholderContext = {
      ...
      clinicalFindings: result.icd10Codes.join(', '),
    ...
    if (totalMs > SLOW_THRESHOLD_MS) {
      logger.warn({ hookInstance, durationMs: totalMs }, 'patient-view handler exceeded 400ms');
    }
    ...
    logInvocation(hookInstance, SERVICE_ID, 'patient-view', context.patientId, cards.length, false, totalMs);
    res.json({ cards } satisfies CdsServiceResponse);
  } catch (err: unknown) {
    logger.error({ err, invocationId }, 'patient-view handler error');
